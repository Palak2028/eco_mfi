...
class AppDatabase {
  ...
  static final tradeNeighbourRefCheckMaster = "Trade_Neighbour_Ref_Check_Master";
...
//Charts Master
  ...
  Future _createDeviationFormMaster(Database db) {
    ...
    print(
        "xxxxxxxxxxxxxxxxxxx ${deviationFormMaster} table Query Here is : " +
            query);
    return db.transaction((Transaction txn) async {
      txn.execute(query);
    });
  ...
  Future updateTradeNeighbourRefCheckMaster(TradeAndNeighbourRefCheckBean bean) async {
    print("trying to insert or replace ${tradeNeighbourRefCheckMaster}");
    String insertQuery = "INSERT OR REPLACE INTO ${tradeNeighbourRefCheckMaster}  "
        "(${TablesColumnFile.trefno} ,"
        "${TablesColumnFile.mrefno} ,"
        "${TablesColumnFile.mloantrefno } ,"
        "${TablesColumnFile.mloanmrefno},"
    ...
    await db.transaction((Transaction txn) async {
      int id = await txn.rawInsert(insertQuery);
      print(id.toString() + " id after insert in ${tradeNeighbourRefCheckMaster}");
    });
  }
...
*/
  ...
  Future<List<GroupFoundationBean>> getGroupFoundationList() async {
    ...
    if (centerNo == "GroupCreation") {
      result = await db.rawQuery(
          "SELECT *  FROM $groupFoundationMaster ");
    }else{
      result = await db.rawQuery(
  ...
  Future<List<StateDropDownList>> getStateList(String countryCd) async {
    ...
    var result;
    print("query is" + "SELECT *  FROM $stateMaster where countryId like '%$countryCd%';");
    result = await db
        .rawQuery("SELECT *  FROM $stateMaster where countryId like '%$countryCd%';");
    try {
      for (int i = 0; i < result.length; i++) {
        print(result[i]);
        for (var items in result[i].toString().split(",")) {
  ...
  Future<List<SubDistrictDropDownList>> getSubDistrictList(bool desendAddress) async {
    var db = await _getDb();
  ...
  Future updateLoginColumn(LoginBean loginBean, String usrCode) async {
    String insertQuery = 'INSERT OR REPLACE INTO '
        ...
        '${TablesColumnFile.mgeolatd} ,'
        '${TablesColumnFile.mgeologd} ,'
        '${TablesColumnFile.agentleftfinger} ,'
        '${TablesColumnFile.agentrightfinger} ,'
        '${TablesColumnFile.mlastsynsdate})'
        ' VALUES('
        ...
        '"${loginBean.mgeolatd}",'
        '"${loginBean.mgeologd}",'
        '"${loginBean.agentleftfinger}",'
        '"${loginBean.agentrightfinger}",'
  ...
  Future createTDRoiInsert(int branch) async {
    ...
    var syncingTDOffsetInterestMasterServices = new SyncingTDOffsetInterestMasterServices();
    ...
    try {
      await syncingTDParameter
          .getTDParameterData(branch)
          .then((onValue) async{
        //TODO for TD Speicific  LBRCode
        print(onValue.length);
        if(onValue!=null){
...
//Customer Formation code start here
  ...
  Future _createPPIMaster(Database db) {
    String createTable = "CREATE TABLE ${PPIMaster} ("
        ...
        " PRIMARY KEY (${TablesColumnFile.trefno}, ${TablesColumnFile
        .mrefno},${TablesColumnFile.tPpirefno})"
        "FOREIGN KEY(${TablesColumnFile
        .mrefno}) REFERENCES ${PPIMaster}(${TablesColumnFile
        .mrefno}) ON DELETE CASCADE,"
        "FOREIGN KEY(${TablesColumnFile
        .trefno}) REFERENCES ${PPIMaster}(${TablesColumnFile
        .trefno}) ON DELETE CASCADE);";
    return db.transaction((Transaction txn) {
  ...
  Future _createCustomerCpvMaster(Database db) {
    String createTableQuery = "CREATE TABLE ${customerCpvMaster} ("
        ...
        " PRIMARY KEY (${TablesColumnFile.trefno}, ${TablesColumnFile
        .mrefno},${TablesColumnFile.tcpvrefno})"
        "FOREIGN KEY(${TablesColumnFile
        .mrefno}) REFERENCES ${customerFoundationMasterDetails}(${TablesColumnFile
        .mrefno}) ON DELETE CASCADE,"
        "FOREIGN KEY(${TablesColumnFile
  ...
  /* Future updateCheckListMaster(CheckListBean checkBean,int idValue) async {
    print("trying toinsert or replace $idValue");
    var db = await _getDb();
    var result = await db.rawQuery(
        'SELECT ${TablesColumnFile.id} FROM ${checkListMaster} where ${TablesColumnFile.trefno} = $idValue');
    bool isUpdate = false;
    int id;
    try {
      if (result[0][TablesColumnFile.trefno].toString() != null) {
        id = result[0][TablesColumnFile.trefno];
        isUpdate = true;
      }
    } catch (_) {
      print("exception here ");
    }
    print("trying to insert or replace ${checkListMaster}");
    String insertQuery = isUpdate
        ?"INSERT OR REPLACE INTO ${checkListMaster}  "
        "("
        "${TablesColumnFile.trefno},"
        "${TablesColumnFile.mrefno},"
        "${TablesColumnFile.mleadsid},"
        ...
        "${TablesColumnFile.mgeolatd},"
        "${TablesColumnFile.mgeologd},"
        "${TablesColumnFile.missync},"
        "${TablesColumnFile.mlastsynsdate}"

        */ /*"${TablesColumnFile.id},"
        "${TablesColumnFile.questionId},"
        "${TablesColumnFile.screenName},"
        "${TablesColumnFile.questionDesc} ,"
        "${TablesColumnFile.loanNumber} ,"
        "${TablesColumnFile.remarks},"
        "${TablesColumnFile.isChecked} ,"
        "${TablesColumnFile.createdAt} ,"
        "${TablesColumnFile.updatedAt} ,"
        "${TablesColumnFile.createdBy}, "
        "${TablesColumnFile.updatedBy} "*/ /*
        ...
        "${checkBean.mrefno},"
        "'${checkBean.mleadsid}' ,"
        "'${checkBean.mquestionid}' ,"
        "${checkBean.manschecked} ,"
        "'${checkBean.mproctype}',"
        "'${checkBean.mcreateddt}' ,"
        "'${checkBean.mcreatedby}' ,"
        "'${checkBean.mlastupdatedt}' ,"
        "'${checkBean.mlastupdateby}' ,"
        "'${checkBean.mgeolocation}' "
        "'${checkBean.mgeolatd}' ,"
        "'${checkBean.mgeologd}' ,"
        "'${checkBean.missync}' ,"
        "'${checkBean.mlastsynsdate}' "
...
/*
  ...
  Future<int> generateTrefNumber() async {
    ...
    String insertQuery = "SELECT *"
        ...
        " WHERE   ${TablesColumnFile.trefno} = (SELECT MAX(${TablesColumnFile
        .trefno})  FROM ${savingsCollectionMaster});";
    var db = await _getDb();
    int retValue;
    try {
      await db.transaction((Transaction txn) async {
...
//Insert Borrowing Basics details
  ...
  //Insert Business Basics details
  Future<int> updateCustomerCpvTable(
      CustomerListBean custBussDetails) async {
    var db = await _getDb();
    String insertQuery =
        "INSERT OR REPLACE INTO ${customerCpvMaster} "
...
// business and house hold expense detail
  ...
  Future<List<KycMasterBean>> selectKycMasterListIsDataSynced(DateTime lastSyncDateTime) async {

    String selectQueryIsDataSynced = lastSyncDateTime != null && !(lastSyncDateTime == 'null')
  ...
  TradeAndNeighbourRefCheckBean bindTradeAndNeighbourRefFormBean(Map<String, dynamic> result) {
    TradeAndNeighbourRefCheckBean tradeAndNeighbourRefCheckBean = new TradeAndNeighbourRefCheckBean();
  ...
  SocialAndEnvironmentalBean bindSocialAndEnvironmentalFormBean(Map<String, dynamic> result) {
    SocialAndEnvironmentalBean socialAndEnvironmentalBean = new SocialAndEnvironmentalBean();
  ...
  Future<List<DisbursmentBean>> selectDisbursmentList(int mcenterid, int mgroupid,
      int mcustno,DateTime fromdate,DateTime todate,routeType ) async {
    var db = await _getDb();
    ...
    String testQuery = "Select ${TablesColumnFile.mleadsid} FROM ${disbursedMaster} ";
  ...
  Future<List<DisbursmentBean>> selectDisbursmentListIsDataSynced(
    ...
    /*  ? 'SELECT * FROM ${savingsMaster}  WHERE ${TablesColumnFile.mcreateddt}  > "$lastSyncDateTime" OR ${TablesColumnFile.mlastupdatedt}  > "$lastSyncDateTime" AND ${TablesColumnFile.trefno}>0'
        : 'SELECT * FROM ${savingsMaster} WHERE ${TablesColumnFile.trefno}>0 ';*/
        ? 'SELECT * FROM ${disbursmentMaster}  WHERE  ${TablesColumnFile
        .mlastupdatedt}  > "$lastSyncDateTime"'
  ...
  Future<List<CenterDetailsBean>> selectCenterListIsDataSynced(
      DateTime lastSyncDateTime) async {
    String selectQueryIsDataSynced = lastSyncDateTime != null &&
        !(lastSyncDateTime == 'null')
        ? 'SELECT * FROM ${centerDetailsMaster}  WHERE  ${TablesColumnFile
        .mlastupdatedt}  > "$lastSyncDateTime"'
        : 'SELECT * FROM ${centerDetailsMaster}';
  ...
  Future<List<TradeAndNeighbourRefCheckBean>> selectTradeAndNeighbourRefListIsDataSynced(
      DateTime lastSyncDateTime) async {
    String selectQueryIsDataSynced = lastSyncDateTime != null &&
        !(lastSyncDateTime == 'null')
        ? 'SELECT * FROM ${tradeNeighbourRefCheckMaster}  WHERE  ${TablesColumnFile
        .mlastupdatedt}  > "$lastSyncDateTime"'
    ...
    var result = await db.rawQuery(selectQueryIsDataSynced);
    print("result" + result.toString());
    List<TradeAndNeighbourRefCheckBean> listbean = new List<TradeAndNeighbourRefCheckBean>();
  ...
  Future<List<SocialAndEnvironmentalBean>> selectSocialAndEnvironmentalListIsDataSynced(
      DateTime lastSyncDateTime) async {
    String selectQueryIsDataSynced = lastSyncDateTime != null &&
        !(lastSyncDateTime == 'null')
        ? 'SELECT * FROM ${socialEnvironmentMaster}  WHERE  ${TablesColumnFile
        .mlastupdatedt}  > "$lastSyncDateTime"'
    ...
    var result = await db.rawQuery(selectQueryIsDataSynced);
    print("result" + result.toString());
    List<SocialAndEnvironmentalBean> listbean = new List<SocialAndEnvironmentalBean>();
  ...
  Future<List<DeviationFormBean>> selectDeviationFormListIsDataSynced(
    ...
    String selectQueryIsDataSynced = lastSyncDateTime != null &&
        !(lastSyncDateTime == 'null')
        ? 'SELECT * FROM ${deviationFormMaster}  WHERE  ${TablesColumnFile
        .mlastupdatedt}  > "$lastSyncDateTime"'
  ...
  Future<SocialAndEnvironmentalBean> getSocialAndEnvValues(int loantrefno, int loanmrefno) async {
    var db = await _getDb();
    ...
    var result = await db.rawQuery('SELECT *  FROM $socialEnvironmentMaster WHERE ${TablesColumnFile.mloantrefno} = ${loantrefno}'
        '  AND ${TablesColumnFile.mloanmrefno} = ${loanmrefno} ');
    try{
      if (result[0] != null) {
        print(result);
  ...
  Future<DeviationFormBean> getDeviationFormValues(int loantrefno, int loanmrefno) async {
    var db = await _getDb();
    ...
    print('xxxxxxxxxxxxxxquery is here : ' +
        'SELECT *  FROM $deviationFormMaster WHERE ${TablesColumnFile.mloantrefno} = ${loantrefno}'
            '  AND ${TablesColumnFile.mloanmrefno} = ${loanmrefno} ');
    var result = await db.rawQuery('SELECT *  FROM $deviationFormMaster WHERE ${TablesColumnFile.mloantrefno} = ${loantrefno}'
        '  AND ${TablesColumnFile.mloanmrefno} = ${loanmrefno} ');
    try{
      if (result[0] != null) {
        print(result);
  ...
  Future<TradeAndNeighbourRefCheckBean> getTradeAndNeighValues(int loantrefno, int loanmrefno) async {
    var db = await _getDb();
    ...
    print('xxxxxxxxxxxxxxquery is here : ' +
        'SELECT *  FROM $tradeNeighbourRefCheckMaster WHERE ${TablesColumnFile.mloantrefno} = ${loantrefno}'
            '  AND ${TablesColumnFile.mloanmrefno} = ${loanmrefno} ');
    var result = await db.rawQuery('SELECT *  FROM $tradeNeighbourRefCheckMaster WHERE ${TablesColumnFile.mloantrefno} = ${loantrefno}'
        '  AND ${TablesColumnFile.mloanmrefno} = ${loanmrefno} ');
    try{
      if (result[0] != null) {
        print(result);
  ...
  Future<List<GroupFoundationBean>> selectGroupListIsDataSynced(
      DateTime lastSyncDateTime) async {
    String selectQueryIsDataSynced = lastSyncDateTime != null &&
        !(lastSyncDateTime == 'null')
        ? 'SELECT * FROM ${groupFoundationMaster}  WHERE  ${TablesColumnFile
  ...
  Future<DisbursmentBean> selectDisbursmentListOnTref(int trefno, String mCreatedBy) async {
    String selectQueryIsDataSynced =
        'SELECT * FROM ${disbursmentMaster}  WHERE ${TablesColumnFile
        .trefno}  = $trefno And ${TablesColumnFile
        .mcreatedby}  = "${mCreatedBy}"';
  ...
  Future<CenterDetailsBean> selectCenterListOnTref(int trefno,
      String mCreatedBy) async {
    String selectQueryIsDataSynced =
        'SELECT * FROM ${centerDetailsMaster}  WHERE ${TablesColumnFile
        .trefno}  = $trefno And ${TablesColumnFile
  ...
  Future<DeviationFormBean> selectDeviationFormListOnTref(int trefno,
      String mCreatedBy) async {
    String selectQueryIsDataSynced =
        'SELECT * FROM ${deviationFormMaster}  WHERE ${TablesColumnFile
        .trefno}  = $trefno And ${TablesColumnFile
  ...
  Future<TradeAndNeighbourRefCheckBean> selectTradeNeighbourRefListOnTref(int trefno,
      String mCreatedBy) async {
    String selectQueryIsDataSynced =
        "SELECT * FROM ${tradeNeighbourRefCheckMaster}  WHERE ${TablesColumnFile
        .trefno}  = $trefno And ${TablesColumnFile
  ...
  Future<SocialAndEnvironmentalBean> selectSocialAndEnvironmentalListOnTref(int trefno,
      String mCreatedBy) async {
    String selectQueryIsDataSynced =
        'SELECT * FROM ${socialEnvironmentMaster}  WHERE ${TablesColumnFile
        .trefno}  = $trefno And ${TablesColumnFile
  ...
  Future<GroupFoundationBean> selectGroupListOnTref(int trefno,
      String mCreatedBy) async {
    String selectQueryIsDataSynced =
        'SELECT * FROM ${groupFoundationMaster}  WHERE ${TablesColumnFile
        .trefno}  = $trefno And ${TablesColumnFile
  ...
  Future<CustomerListBean> selectCustomerOnTref(int trefno,
      String mCreatedBy) async {
    String selectQueryIsDataSynced =
        'SELECT * FROM ${customerFoundationMasterDetails}  WHERE ${TablesColumnFile
        .trefno}  = $trefno And ${TablesColumnFile
  ...
  Future<CustomerBusinessDetailsBean> selectCustomerBussinessDetails(int tRefNo,
      int mRefNo) async {
    var db = await _getDb();
  ...
  Future<ContactPointVerificationBean> selectCustomerCpvDetails(int tRefNo,
      int mRefNo) async {
    var db = await _getDb();
    String selectQuery =
        "SELECT * FROM ${customerCpvMaster}  WHERE ${TablesColumnFile
        .trefno}  = $tRefNo AND  ${TablesColumnFile.mrefno} = $mRefNo";
    print("select query is ${selectQuery} ");
    var result = await db.rawQuery(selectQuery);
  ...
  Future updateKycListMaster(String query) async {
    ...
    await db.transaction((Transaction txn) async {
      await txn
          .rawQuery("UPDATE ${kycMaster} SET ${query}");
    });
  }
  ...
  Future<KycMasterBean> selectKycMasterOnTrefAndMrefNo(int trefno,
      int mrefno) async {


    String selectQueryIsDataSynced =
        'SELECT * FROM ${kycMaster}  WHERE ${TablesColumnFile
        .trefno}  = $trefno  AND ${TablesColumnFile.mrefno}  = ${mrefno}';
    ...
    if (result.isEmpty) {
      ...
      'SELECT * FROM ${kycMaster}  WHERE ${TablesColumnFile
          .trefno}  = $trefno  AND ${TablesColumnFile.mrefno}  = 0 ';
      print(selectQueryIsDataSynced);
      result = await db.rawQuery(selectQueryIsDataSynced);
    }
  ...
  getSunPurposeOfLoanListFromSubLookpTable(int hardcodedValue,
    ...
    result = await db.rawQuery(
        'SELECT *  FROM $SubLookupMaster where ${TablesColumnFile
            .mcodetype}  = ${hardcodedValue + position};');
    try {
      for (int i = 0; i < result.length; i++) {
...
*/
  ...
  Future<String> getImageStringPath(String imageString) async {
    ...
    if (imageString != null &&
      ...
      final String dirPath =
          '${extDir.path}/Pictures/eco_mfi/syncedNOCImage';
  ...
  Future<Null> updateStaticTablesLastSyncedMaster(int idValue) async {
    ...
    print( "UPDATE ${lastSyncedDateTimeMaster} SET ${TablesColumnFile
        .tlastSyncedFromTab} = '${DateTime.now()}' WHERE ${TablesColumnFile
        .id} = ${idValue}");
  }
...
*/
  ...
  Future<List<PPIMasterBean>> selectCustomerPPIListIsDataSynced(int tRefNo,
      int mRefNo) async {
    var db = await _getDb();

    String selectQuery =
        "SELECT * FROM ${PPIMaster}  WHERE ${TablesColumnFile
        .trefno}  = $tRefNo AND  ${TablesColumnFile.mrefno} = $mRefNo";
    print("select query is ${selectQuery} ");
    var result = await db.rawQuery(
        "SELECT * FROM ${PPIMaster}  WHERE ${TablesColumnFile
...
// according to number of days and number of months PLUS Amount
  ...
  Future<List<GuarantorDetailsBean>> getGuarantorsNotSynced(
    ...
    String selectQueryIsDataSynced = lastSyncDateTime != null &&
        !(lastSyncDateTime == 'null')
        ? 'SELECT * FROM ${gaurantorMaster}  WHERE  '
        '${TablesColumnFile.mcreateddt}  > "$lastSyncDateTime" OR ${TablesColumnFile.mlastupdatedt}  > "$lastSyncDateTime" '
  ...
  Future<int> generateSrNoForGuarantor(int loantrefno) async {
    ...
    String insertQuery = "SELECT *""FROM    ${gaurantorMaster}"" WHERE   ${TablesColumnFile.msrno} = (SELECT MAX(${TablesColumnFile.msrno})  FROM ${gaurantorMaster} where ${TablesColumnFile.mloantrefno}=${loantrefno});";
  ...
  Future<int> generateTrefnoForCenterCreation() async {
    ...
    String insertQuery = "SELECT *""FROM    ${centerDetailsMaster}"" WHERE   ${TablesColumnFile.trefno} = (SELECT MAX(${TablesColumnFile.trefno})  FROM ${centerDetailsMaster});";
    var db = await _getDb();
    int retValue;
    try {
      await db.transaction((Transaction txn) async {
  ...
  Future<int> generateTradeAndNeighbourRef() async {
    ...
    String insertQuery = "SELECT *""FROM    ${tradeNeighbourRefCheckMaster}"" WHERE   ${TablesColumnFile.trefno} = (SELECT MAX(${TablesColumnFile.trefno})  FROM ${tradeNeighbourRefCheckMaster});";
    var db = await _getDb();
    int retValue;
    try {
      await db.transaction((Transaction txn) async {
  ...
  Future<int> generateSocialEnvironment() async {
    ...
    String insertQuery = "SELECT *""FROM    ${socialEnvironmentMaster}"" WHERE   ${TablesColumnFile.trefno} = (SELECT MAX(${TablesColumnFile.trefno})  FROM ${socialEnvironmentMaster});";
    var db = await _getDb();
    int retValue;
    try {
      await db.transaction((Transaction txn) async {
  ...
  Future<int> generateDeviationForm() async {
    ...
    String insertQuery = "SELECT *""FROM    ${deviationFormMaster}"" WHERE   ${TablesColumnFile.trefno} = (SELECT MAX(${TablesColumnFile.trefno})  FROM ${deviationFormMaster});";
    var db = await _getDb();
    int retValue;
    try {
      await db.transaction((Transaction txn) async {
  ...
  Future<int> getCenterRepayFromTo() async {
    ...
    String insertQuery = "SELECT *""FROM    ${SystemParameterMaster}"" WHERE   ${TablesColumnFile.trefno} = (SELECT MAX(${TablesColumnFile.trefno})  FROM ${centerDetailsMaster});";
    var db = await _getDb();
    int retValue;
    try {
      await db.transaction((Transaction txn) async {
  ...
  Future<int> generateTrefnoForGroupCreation() async {
    ...
    String insertQuery = "SELECT *""FROM    ${groupFoundationMaster}"" WHERE   ${TablesColumnFile.trefno} = (SELECT MAX(${TablesColumnFile.trefno})  FROM ${groupFoundationMaster});";
    var db = await _getDb();
    int retValue;
    try {
      await db.transaction((Transaction txn) async {
  ...
  Future<CustomerListBean> getCustMrefAndTref(int custNo) async {
    ...
    result = await db.rawQuery(
        "SELECT *  FROM $customerFoundationMasterDetails WHERE ${TablesColumnFile
            .mcustno} = $custNo ;");
    print("SELECT *  FROM $customerFoundationMasterDetails WHERE ${TablesColumnFile
        .mcustno} = $custNo ;" + result.toString() + result.length.toString() );
    try {
      for (int i = 0; i < result.length; i++) {
  ...
  Future<TDParameterBean> getParameterForProduct(String prdCd,int lbrCd) async {
  ...
  Future<List<DisbursmentBean>> getDisbursedListForIndvPrint(int CustNo) async {
    try {
      ...
      var result;
      print("query is" +
          "SELECT *  FROM $disbursedMaster WHERE ${TablesColumnFile
              .mcustno} = $CustNo ;");
  ...
  Future<CustomerLoanDetailsBean> selectCustomerLoanOnTrefAndMref(int trefno,
      String mCreatedBy,int mrefno) async {
    String selectQueryIsDataSynced =
        'SELECT * FROM ${customerLoanDetailsMaster}  WHERE ${TablesColumnFile
        .trefno}  = $trefno And ${TablesColumnFile
        .mcreatedby}  = "${mCreatedBy}" AND ${TablesColumnFile
        .mrefno}  = $mrefno';
...
*/
  ...
  Future<List<ImageBean>> getFingerListByCustNo(int mcustno) async {
    ...
    await db.rawQuery(
        'SELECT * FROM ${imageMaster} where ${TablesColumnFile
            .mrefno} = (Select ${TablesColumnFile.mrefno} from ${customerFoundationMasterDetails} '
  ...
  Future<List<UserRightBean>> getUserRights(UserRightBean getBean, String menuType) async {
    try {
      var db = await _getDb();
  ...
  Future updateUserRightsMaster(
      UserRightBean userRightBean) async {
  ...
  Future updateMenuMaster(
      MenuMasterBean menuMasterBean) async {
  ...
  Future<AddressDescBean> getAddressHererchyInDescOrder(String placeCd) async {
    try {
      var db = await _getDb();
      ...
      //City
      String queryForGetMenu = 'SELECT *  FROM $subDistrictMaster where placeCd = $placeCd;';
      var result;
      result = await db.rawQuery(queryForGetMenu);
      ...
      //District
      queryForGetMenu = 'SELECT *  FROM $districtMaster where distCd = ${subDistBean.distCd};';
      result = await db.rawQuery(queryForGetMenu);
      ...
      //State
      queryForGetMenu = 'SELECT *  FROM $stateMaster where stateCd = "${distBean.stateCd}";';
      result = await db.rawQuery(queryForGetMenu);
      ...
      //Country
      queryForGetMenu = 'SELECT *  FROM $countryMaster where countryId = "${stateDropDownList.cntryCd}";';
      result = await db.rawQuery(queryForGetMenu);
  ...
  Future upadateLoanLevelMaster(
      LoanLevel loanLevel) async {
  ...
  Future<List<LoanLevel>> getLoanLevelForStage(int stageid,String mprdCd) async {
  ...
  Future updateCustomerLoanCashFlowMaster(CustomerLoanCashFlowAnalysisBean loanCahFlowBean) async {
  ...
  Future<CustomerLoanCashFlowAnalysisBean> getCashFlowAnalysis(int loantrefno, int loanmrefno) async {
    var db = await _getDb();
    // var adhaarNo = "${TablesColumnFile.adhaarNo} = ${creditBereauBean.adhaarNo}";
    CustomerLoanCashFlowAnalysisBean retBean = new CustomerLoanCashFlowAnalysisBean();

    print('xxxxxxxxxxxxxxquery is here : ' +
        'SELECT *  FROM $CosutomerLoanCashFlowMaster WHERE ${TablesColumnFile.mloantrefno} = ${loantrefno}'
            '  AND ${TablesColumnFile.mloanmrefno} = ${loanmrefno} ');
    var result = await db.rawQuery('SELECT *  FROM $CosutomerLoanCashFlowMaster WHERE ${TablesColumnFile.mloantrefno} = ${loantrefno}'
        '  AND ${TablesColumnFile.mloanmrefno} = ${loanmrefno} ');
    try{
  ...
  Future<int> getMaxtrefNumber() async {
    String insertQuery = "SELECT *"
        ...
        " WHERE   ${TablesColumnFile.trefno} = (SELECT MAX(${TablesColumnFile
        .trefno})  FROM ${CosutomerLoanCashFlowMaster});";
    var db = await _getDb();
    int retValue;
    try {
      await db.transaction((Transaction txn) async {
  ...
  Future<List<CustomerLoanCashFlowAnalysisBean>> selectCashFlowListNotSynced(
    ...
    var db = await _getDb();
    CustomerLoanCashFlowAnalysisBean retBean = new CustomerLoanCashFlowAnalysisBean();
    List<CustomerLoanCashFlowAnalysisBean> n = new List<CustomerLoanCashFlowAnalysisBean>();
    var result;
    //if(future!=null) {

    String selectQueryIsDataSynced = lastSyncDateTime != null &&
        !(lastSyncDateTime == 'null')
        ? 'SELECT * FROM ${CosutomerLoanCashFlowMaster}  WHERE  ${TablesColumnFile
        .mlastupdatedt}  > "$lastSyncDateTime"'
  ...
  Future<CustomerLoanCashFlowAnalysisBean> selectCashFlowonTrefAndMref(int trefno,
      String mCreatedBy,int mrefno) async {
    String selectQueryIsDataSynced =
        'SELECT * FROM ${CosutomerLoanCashFlowMaster}  WHERE ${TablesColumnFile
        .trefno}  = $trefno And ${TablesColumnFile
  ...
  Future _createKycMaster(Database db) {
    ...
    print("xxxxxxxxxxxxxxxxxxx ${kycMaster} table Query Here is : " +
        query);
    return db.transaction((Transaction txn) async {
      txn.execute(query);
    });
...
//kyc master Tref genration
  Future<int> generateTrefnoForKycMaster() async {
    ...
    String insertQuery = "SELECT * FROM    ${kycMaster}  WHERE   ${TablesColumnFile.trefno} = (SELECT MAX(${TablesColumnFile.trefno})  FROM ${kycMaster});";
    var db = await _getDb();
    int retValue;
    try {
      await db.transaction((Transaction txn) async {
  ...
  Future _createLoanCPVBusinessRecord(Database db) {
    ...
    print("xxxxxxxxxxxxxxxxxxx ${LoanCPVBusinessRecord} table Query Here is : " +
        query);
    return db.transaction((Transaction txn) async {
      txn.execute(query);
    });
  ...
  Future _createCustomerLoanImageMaster(Database db) {
    ...
    print("xxxxxxxxxxxxxxxxxxx ${customerLoanImageMaster} table Query Here is : " +
        query);
    return db.transaction((Transaction txn) async {
      txn.execute(query);
    });
...
//Genrate TREFNO for Loan CPV
  Future<int> generateTrefnoForLoanCPVBusinessRecord() async {
    ...
    String insertQuery = "SELECT * FROM    ${LoanCPVBusinessRecord}  WHERE   ${TablesColumnFile.trefno} = (SELECT MAX(${TablesColumnFile.trefno})  FROM ${LoanCPVBusinessRecord});";
    var db = await _getDb();
    int retValue;
    try {
      await db.transaction((Transaction txn) async {
  ...
  Future<int> updateLoanCPVBusinessRecord( CustomerLoanCPVBusinessRecordBean loanCPVBusinessRecordObj) async {
  ...
  Future<CustomerLoanCPVBusinessRecordBean> selectLoanCPVBusinessRecordOnTref(int trefno,
      String mCreatedBy) async {
    String selectQueryIsDataSynced =
        "SELECT * FROM ${LoanCPVBusinessRecord}  WHERE ${TablesColumnFile
        .trefno}  = $trefno And ${TablesColumnFile
        .mcreatedby}  = '${mCreatedBy}'";
  ...
  Future<List<CustomerLoanCPVBusinessRecordBean>> selectLoanCPVBusinessRecordIsDataSynced(
      DateTime lastSyncDateTime) async {
    String selectQueryIsDataSynced = lastSyncDateTime != null &&
        !(lastSyncDateTime == 'null')
        ? 'SELECT * FROM ${LoanCPVBusinessRecord}  WHERE  ${TablesColumnFile
    ...
    var result = await db.rawQuery(selectQueryIsDataSynced);
    print("result" + result.toString());
    List<CustomerLoanCPVBusinessRecordBean> listbean = new List<CustomerLoanCPVBusinessRecordBean>();
    CustomerLoanCPVBusinessRecordBean bean;
    try {
      for (int i = 0; i < result.length; i++) {
        bean = new CustomerLoanCPVBusinessRecordBean();
  ...
  Future<CustomerLoanCPVBusinessRecordBean> getLoanCPVValues(int loantrefno, int loanmrefno) async {
    var db = await _getDb();
    // var adhaarNo = "${TablesColumnFile.adhaarNo} = ${creditBereauBean.adhaarNo}";
    CustomerLoanCPVBusinessRecordBean retBean = new CustomerLoanCPVBusinessRecordBean();

    print('xxxxxxxxxxxxxxquery is here : ' +
        'SELECT *  FROM $LoanCPVBusinessRecord WHERE ${TablesColumnFile.mloantrefno} = ${loantrefno}'
            '  AND ${TablesColumnFile.mloanmrefno} = ${loanmrefno} ');
    var result = await db.rawQuery('SELECT *  FROM $LoanCPVBusinessRecord WHERE ${TablesColumnFile.mloantrefno} = ${loantrefno}'
        '  AND ${TablesColumnFile.mloanmrefno} = ${loanmrefno} ');
    try{
  ...
  CustomerLoanCPVBusinessRecordBean bindLoanCPVBusinessRecordBean(Map<String, dynamic> result) {
    CustomerLoanCPVBusinessRecordBean customerLoanCPVBusinessRecordBean = new CustomerLoanCPVBusinessRecordBean();
  ...
  Future updateCashFlowMaster(String query) async {
    ...
    print("Update cashFlow query is UPDATE ${CosutomerLoanCashFlowMaster} SET ${query} ");
  }

  Future<CustomerLoanCashFlowAnalysisBean> selectCashFlowonLoanTrefLoanmerefno(int mloanterfno,
      int mloanmrefno,bool bothCond) async {
    String selectQueryIsDataSynced =
        "SELECT * FROM ${CosutomerLoanCashFlowMaster}  WHERE ${TablesColumnFile
        .mloantrefno}  = $mloanterfno And ${TablesColumnFile
        .mloanmrefno}  = ${mloanmrefno} "  ;
...
<<<<<<< .mine
      "SELECT * FROM ${CosutomerLoanCashFlowMaster}  WHERE ${TablesColumnFile
          .mloanterfno}  = $mloanterfno ${TablesColumnFile
=======
      "SELECT * FROM ${CosutomerLoanCashFlowMaster}  WHERE ${TablesColumnFile
          .mloantrefno}  = $mloanterfno AND ${TablesColumnFile
>>>>>>> .r1143
  ...
  Future<KycMasterBean> getKycMasterObj(int loantrefno, int loanmrefno) async {
    ...
    print('xxxxxxxxxxxxxxquery is here : ' +
        'SELECT *  FROM $kycMaster WHERE ${TablesColumnFile.mloantrefno} = ${loantrefno}'
            '  AND ${TablesColumnFile.mloanmrefno} = ${loanmrefno} ');
    var result = await db.rawQuery('SELECT *  FROM $kycMaster WHERE ${TablesColumnFile.mloantrefno} = ${loantrefno}'
        '  AND ${TablesColumnFile.mloanmrefno} = ${loanmrefno} ');
    try{
...
<<<<<<< .mine
  Future<DeviationFormBean> selectDeviationFormTrefLoanmerefno(int mloanterfno,
      int mloanmrefno) async {
    String selectQueryIsDataSynced =
        "SELECT * FROM ${deviationFormMaster}  WHERE ${TablesColumnFile
        .mloanterfno}  = $mloanterfno And ${TablesColumnFile
        .mloanmrefno}  = ${mloanmrefno} "  ;
=======
  Future<DeviationFormBean> selectDeviationFormTrefLoanmerefno(int mloanterfno,
      int mloanmrefno,bool bothCondition) async {
    String selectQueryIsDataSynced =
        "SELECT * FROM ${deviationFormMaster}  WHERE ${TablesColumnFile
        .mloantrefno}  = $mloanterfno And ${TablesColumnFile
        .mloanmrefno}  = ${mloanmrefno} "  ;
...
<<<<<<< .mine
  Future<SocialAndEnvironmentalBean> selectSocialAndEnvTrefLoanmerefno(int mloanterfno,
      int mloanmrefno) async {
    String selectQueryIsDataSynced =
        "SELECT * FROM ${socialEnvironmentMaster}  WHERE ${TablesColumnFile
        .mloanterfno}  = $mloanterfno And ${TablesColumnFile
  ...
  Future<TradeAndNeighbourRefCheckBean> selectTradeAndNeighTrefLoanmerefno(int mloanterfno,
      int mloanmrefno) async {
    String selectQueryIsDataSynced =
        "SELECT * FROM ${tradeNeighbourRefCheckMaster}  WHERE ${TablesColumnFile
        .mloanterfno}  = $mloanterfno And ${TablesColumnFile
  ...
  Future<CustomerLoanCPVBusinessRecordBean> selectCPVTrefLoanmerefno(int mloanterfno,
      int mloanmrefno) async {
    String selectQueryIsDataSynced =
        "SELECT * FROM ${LoanCPVBusinessRecord}  WHERE ${TablesColumnFile
        .mloanterfno}  = $mloanterfno And ${TablesColumnFile
  ...
  Future<SubLookupForSubPurposeOfLoan>
  getOccupation(int hardcodedValue, int position,String  code) async {
    var db = await _getDb();
    SubLookupForSubPurposeOfLoan retBean = new SubLookupForSubPurposeOfLoan();
    print("SELECT *  FROM $SubLookupMaster where ${TablesColumnFile
        .mcodetype}  = ${hardcodedValue + position}   AND   ${TablesColumnFile.mcode} = '${code}' ");
    var result;
    result = await db.rawQuery(
        "SELECT *  FROM $SubLookupMaster where ${TablesColumnFile
            .mcodetype}  = ${hardcodedValue + position}   AND   ${TablesColumnFile.mcode} = '${code}' ");
  ...
  Future<int> updateAgentBiometric (String userCode, String fingerType,
      ...
      String agentrightfinger) async {
    var db = await _getDb ();
    String query;
    if (fingerType == "LH") {
      query =
      "Update $userMasterTable SET ${TablesColumnFile.agentleftfinger} "
          "= '${agentleftfinger}'"
  ...
  Future<LoginBean> getAgentFingerPrint () async {
    ...
    var result =
    await db.rawQuery ('SELECT ${TablesColumnFile.agentleftfinger},${TablesColumnFile.agentrightfinger}'
        ' FROM $userMasterTable ');
    try {
      if (result[0] != null) {
        print (result[0]);
...
=======
  ...
  Future<GuarantorDetailsBean> selectGuarantorTrefLoanmerefno(int mloanterfno,
      int mloanmrefno,bool bothCodition) async {
    String selectQueryIsDataSynced =
  ...
  Future<List<CustomerLoanImageBean>> selectCustomerLoanImage(int tRefNo,
      int mRefNo) async {
    var db = await _getDb();
    var result = await db.rawQuery(
  ...
  CustomerLoanImageBean bindLoanDataImagessListBean(Map<String, dynamic> result) {
  ...
  Future<int> updateCustomerLoanImageMaster( CustomerLoanImageBean loanImageBean, int tImagerefNo) async {
    print("trying to insert or replace ${customerLoanImageMaster}");
    String insertQuery = 'INSERT OR REPLACE INTO ${customerLoanImageMaster} '
        "(${TablesColumnFile.trefno} ,"
        "${TablesColumnFile.mrefno} ,"
        "${TablesColumnFile.mloantrefno } ,"
        "${TablesColumnFile.mloanmrefno},"
        ...
        "'${loanImageBean.missynctocoresys}' "
        ");";
    print("insert query is" + insertQuery);
    ...
    await db.transaction((Transaction txn) async {
      int id = await txn.rawInsert(insertQuery);
      print(id.toString() + " id after insert in ${customerLoanImageMaster}");
    });
  }
  ...
  Future<CustomerLoanImageBean> selectCustomerLoanImageOnmreftref(int mloanterfno,
      int mloanmrefno) async {
    String selectQueryIsDataSynced =
        "SELECT * FROM ${customerLoanImageMaster}  WHERE ${TablesColumnFile
        .mloantrefno}  = $mloanterfno And ${TablesColumnFile
        .mloanmrefno}  = ${mloanmrefno} "  ;
  ...
  Future<List<CustomerLoanImageBean>> selectCustomerLoanImageNotSynced(
      DateTime lastSyncDateTime) async {
    var db = await _getDb();
    ...
    String selectQueryIsDataSynced = lastSyncDateTime != null &&
        ...
        ? 'SELECT * FROM ${customerLoanImageMaster}  WHERE  ${TablesColumnFile
        .mlastupdatedt}  > "$lastSyncDateTime"'
        : 'SELECT * FROM ${customerLoanImageMaster}';
    result = await db.rawQuery(selectQueryIsDataSynced);
  ...
  Future<List<CustomerLoanImageBean>> getCustomerLoanImage(int mrefno, int trefno) async {
    var db = await _getDb();
    CustomerLoanImageBean retBean = new CustomerLoanImageBean();
    List<CustomerLoanImageBean> customerLoanImageList = new List<CustomerLoanImageBean>();
    String query =
        "SELECT *  FROM $customerLoanImageMaster  WHERE  ${TablesColumnFile
        .mloanmrefno} = ${mrefno} AND ${TablesColumnFile
        .mloantrefno} = ${trefno}";
    var result;
  ...
  Future<int> getMaxCustomerLoanImageNumber() async {
    String insertQuery = "SELECT *"
        ...
        " WHERE   ${TablesColumnFile.trefno} = (SELECT MAX(${TablesColumnFile
        .trefno})  FROM ${customerLoanImageMaster});";
    var db = await _getDb();
    int retValue;
    try {
      await db.transaction((Transaction txn) async {
  ...
  Future<List<LastSyncDateTimeBean>> getDateTime() async {
    ...
    String selectQueryIsDataSynced  = 'SELECT * FROM ${lastSyncedDateTimeMaster} ';
    result = await db.rawQuery(selectQueryIsDataSynced);
  ...
  Future<KycMasterBean> selectKycMasterTrefLoanmerefno(int mloanterfno,
      int mloanmrefno) async {
    String selectQueryIsDataSynced =
        "SELECT * FROM ${kycMaster}  WHERE ${TablesColumnFile
        .mloantrefno}  = $mloanterfno And ${TablesColumnFile
  ...
  Future<CustomerListBean> getDedupCustomerIfPresent(String nid, DateTime mdob) async {
    var db = await _getDb();
    ...
    String selectCustQuery = "SELECT *FROM ${customerFoundationMasterDetails} where ${TablesColumnFile.mdob} = '$mdob' AND   substr(mpannodesc,length(mpannodesc)-5,6) ='$nidSubs';";
